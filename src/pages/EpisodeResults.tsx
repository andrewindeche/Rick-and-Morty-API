  ...
  async function handleSearch(query: string) {
    try {
      ...
      if (episodeResponse.data.results.length > 0) {
        setEpisodeResults(episodeResponse.data.results);
        const characterPromises = episodeResponse.data.results.map((episode: any) =>
          episode.characters.map((character: string) => axios.get(character))
        );
        const characterResponses = await Promise.all(characterPromises.flat());
                ...
                <div className="header">
                  ...
                  {characters.length > 0 && (
                    <div className="ResidentsHeader">
                      EPISODE RESULTS: <span className='headername'>{characters.length}</span>
                    </div>
                  )}
                </div>
              )}
              {characters.length > 0 ? (
                <div className="results">{characters.length} RESULT{characters.length > 1 ? 'S' : ''} FOR EPISODE</div>
              ) : (
